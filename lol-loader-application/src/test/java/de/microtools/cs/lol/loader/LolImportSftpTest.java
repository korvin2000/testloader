...
import de.microtools.cs.lol.loader.application.ftp.SFTPClient;
...
public class LolImportSftpTest extends LolImportCommonTest {

   private static final Logger logger = LoggerFactory.getLogger(LolImportSftpTest.class);

   public static int port = findFreePort();

   @Rule
   public final FakeSftpServerRule sftpServer = new FakeSftpServerRule().setPort(port).addUser("spring", "spring");
   ...
   public static void initBeforeClass() throws Exception {
      ...
      System.setProperty("lol.ftp.useSftp", "true");
   ...
   public void setUp() throws Exception {
      ...
      for (TestSource testFile : testFiles) {
         sftpServer.putFile(FAKE_FOLDER + testFile.getResource().getFile().getName(), IOUtils.toString(testFile.getResource().getURI()), UTF_8);
   ...
   public static void tearDownClass() throws Exception {
      System.setProperty("lol.ftp.useSftp", "false");
      System.setProperty("lol.ftp.port", "21");
      Thread.sleep(250);
   }
   ...
   public void testSftpConnection() throws Exception {
      ...
      File initialFile = new File("src/test/resources/data/123456789001_100_1100_150828.xml");
      InputStream targetStream = new FileInputStream(initialFile);
      File outputFile = new File("target/ftp-in/test.xml");
      sftpServer.createDirectory("/test");
      ...
      sftpClient.upload("/test/test.xml", targetStream);
      sftpClient.listNames("/test");
      sftpClient.download("/test");
      sftpClient.downloadFile("/test/test.xml", new FileOutputStream(outputFile));
      sftpClient.delete("/test/test.xml");
      outputFile.delete();
      sftpClient.close();
   }
   ...
   public void testInvalidSchemaIncomingFile() throws Exception {
      ...
      assertThat("Invalid Rueckliferung "+ invalidSchema.getResource().getFilename() +" not deleted from work dir",
            new UrlResource(workDir + "/"+ invalidSchema.getResource().getFilename())
            .exists(),
            is(false));
   ...
   public void testNotParsableIncomingFile() throws Exception {
      ...
      assertThat("Invalid Rueckliferung "+ notParseable.getResource().getFilename() +" not deleted from work dir",
            new UrlResource(workDir + "/"+ notParseable.getResource().getFilename())
            .exists(),
            is(false));
   }
